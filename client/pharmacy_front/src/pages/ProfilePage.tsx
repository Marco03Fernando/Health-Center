...
export default function ProfilePage() {
  ...
  useEffect(() => {
    // initialize from auth context, fallback to localStorage
    const storageUser = (() => {
      try {
    ...
    // Prefill name for convenience, but leave email input empty so user must enter it
    setName(admin?.name || storageUser?.name || "");
    setEmail("");
  }, [admin]);
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm text-muted-foreground md:text-base">
                Manage your pharmacist account details, keep profile information
            ...
            <CardContent className="p-5">
              <div className="flex items-center gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <Badge className="mt-2 rounded-full px-3 py-1 capitalize">{admin?.role || "pharmacist"}</Badge>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm font-semibold capitalize">{admin?.role || "pharmacist"}</p>
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      <div className="grid gap-5 xl:grid-cols-[1.35fr_0.9fr]">
        <Card className="rounded-3xl border shadow-sm">
          <CardHeader className="pb-3">
            ...
            <p className="text-sm text-muted-foreground">Update your name and email used for the account.</p>
          </CardHeader>
            ...
            <div className="rounded-2xl border bg-muted/20 p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">{hasChanges ? "You have unsaved changes in your profile." : "Your profile information is up to date."}</p>
                </div>

                <Badge
                  variant="secondary"
                  className={`rounded-full px-3 py-1 ${hasChanges ? "bg-amber-500/10 text-amber-700 dark:text-amber-400" : "bg-emerald-500/10 text-emerald-700 dark:text-emerald-400"}`}
                >
                    ...
                    <div className="space-y-5 py-5">
                      ...
                      <div className="rounded-2xl border bg-muted/20 p-4">
                        <div className="flex items-start gap-3">
                          ...
                          <p className="text-sm text-muted-foreground">Use a strong password with a mix of letters, numbers, and symbols for better security.</p>
                        </div>
                      </div>

                      <div className="flex justify-end">
                        <Button onClick={handleChangePw} className="h-11 rounded-xl px-5">Update Password</Button>
                      </div>
                    </div>
                  </div>
                </DialogContent>
        ...
        <div className="space-y-5">
          <Card className="rounded-3xl border shadow-sm">
            <CardHeader className="pb-3">
              ...
              <p className="text-sm text-muted-foreground">Quick overview of your account.</p>
            </CardHeader>

            <CardContent className="space-y-3 pt-3">
              <div className="flex items-center justify-between rounded-2xl border bg-muted/20 px-4 py-3">
                <span className="text-sm text-muted-foreground">Full Name</span>
                <span className="max-w-[55%] truncate text-sm font-semibold">{admin?.name || "-"}</span>
              </div>

              <div className="flex items-center justify-between rounded-2xl border bg-muted/20 px-4 py-3">
                <span className="text-sm text-muted-foreground">Email</span>
                <span className="max-w-[55%] truncate text-sm font-semibold">{admin?.email || "-"}</span>
              </div>

              <div className="flex items-center justify-between rounded-2xl border bg-muted/20 px-4 py-3">
                <span className="text-sm text-muted-foreground">Role</span>
                <span className="text-sm font-semibold capitalize">{admin?.role || "pharmacist"}</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-3xl border shadow-sm">
            <CardHeader className="pb-3">
              ...
              <p className="text-sm text-muted-foreground">Basic account safety recommendations.</p>
            </CardHeader>

            <CardContent className="space-y-3 pt-3">
              <div className="rounded-2xl border bg-muted/20 p-4">
                <p className="text-sm font-medium">Password protection</p>
                <p className="mt-1 text-sm text-muted-foreground">Change your password regularly and avoid reusing old passwords.</p>
              </div>

              <div className="rounded-2xl border bg-muted/20 p-4">
                <p className="text-sm font-medium">Account identity</p>
                <p className="mt-1 text-sm text-muted-foreground">Make sure your displayed name and email stay accurate for activity.</p>
              </div>
            </CardContent>
          </Card>
        </div>
