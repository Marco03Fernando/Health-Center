...
import { userLogin, userRegister, userLogout as apiUserLogout, userGetCurrentUser, } from "@/services/user-auth.service";
// ─── Context ──────────────────────────────────────────────────────────────────
const UserAppContext = createContext(null);
export const useUserApp = () => {
    const ctx = useContext(UserAppContext);
    if (!ctx)
        throw new Error("useUserApp must be inside UserAppProvider");
    return ctx;
};
...
export const UserAppProvider = ({ children }) => {
    ...
    if (isAuthLoading)
        return null;
    return (<UserAppContext.Provider value={{
            user,
            cart,
            orders,
            isAuthLoading,
