...
const CartPage = () => {
    const { cart, updateCartQty, removeFromCart, placeOrder } = useUserApp();
    ...
    if (step === 'success') {
        ...
        <div className="flex gap-3">
          ...
          <Button variant="outline" onClick={() => navigate('/user/marketplace')}>Continue Shopping</Button>
        </div>
      </div>);
    }
    if (cart.length === 0) {
    ...
    return (<div className="space-y-5 animate-fade-in">
      ...
      {step === 'cart' && (<>
          <div className="space-y-3">
            {cart.map(item => (<Card key={item.product.id}>
