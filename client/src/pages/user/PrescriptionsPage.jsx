...
function mapPrescription(rx) {
    const doctor = typeof rx.doctorId === "object" && rx.doctorId !== null ? rx.doctorId : null;
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <div className="space-y-2">
              {rx.medicines.map((m, i) => (<div key={i} className="p-3 rounded-lg bg-muted">
                  <p className="font-medium text-sm text-foreground">{m.name}</p>
                  <p className="text-xs text-muted-foreground">
                    {m.dosage} · {m.frequency} · {m.duration}
                  </p>
                  {m.instructions ? (<p className="text-xs text-muted-foreground mt-1">
                      {m.instructions}
                    </p>) : null}
                </div>))}
            </div>
...
const PrescriptionsPage = () => {
    ...
    if (selectedId && selectedRx) {
        return (<PrescriptionDetail rx={selectedRx} onBack={() => {
                setSelectedId(null);
                setSelectedRx(null);
            }}/>);
    }
    return (<div className="space-y-5 animate-fade-in">
      ...
      {loading ? (<div className="text-center py-12 text-muted-foreground">
        ...
        </div>) : prescriptions.length === 0 ? (<div className="text-center py-12 text-muted-foreground">
          No prescriptions yet.
        </div>) : (<div className="grid sm:grid-cols-2 gap-4">
          {prescriptions.map((rx) => (<Card key={rx.id} className="cursor-pointer hover:shadow-md transition-shadow" onClick={() => setSelectedId(rx.id)}>
      ...
      {loadingDetail ? (<div className="text-center py-4 text-muted-foreground">
