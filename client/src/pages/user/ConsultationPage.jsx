...
const DoctorsPage = () => {
    ...
    const doctorSuggestions = useMemo(() => {
        return doctors.filter((d) => {
            const centerName = typeof d.centerId === "object"
    ...
    return (<div className="min-h-[80vh] flex flex-col justify-center animate-fade-in">
      <div className="max-w-5xl mx-auto w-full space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr_auto] gap-4 items-end">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input list="center-options" value={centerSearch} onChange={(e) => setCenterSearch(e.target.value)} placeholder={loadingCenters ? "Loading centers..." : "Search center..."} className="pl-10 h-11"/>
                  <datalist id="center-options">
                    {centerSuggestions.map((center) => (<option key={center} value={center}/>))}
                  </datalist>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input list="doctor-options" value={doctorSearch} onChange={(e) => setDoctorSearch(e.target.value)} placeholder="Search doctor or specialization..." className="pl-10 h-11"/>
                  <datalist id="doctor-options">
                    {doctorSuggestions.map((doctor) => (<option key={doctor._id} value={doctor.name}>{`${doctor.name} - ${doctor.specialization}`}</option>))}
                  </datalist>
                </div>
              </div>

              <Button onClick={handleSearch} className="h-11 px-6 w-full md:w-auto" disabled={loadingDoctors}>
                {loadingDoctors ? (<Loader2 className="w-4 h-4 mr-2 animate-spin"/>) : (<Search className="w-4 h-4 mr-2"/>)}
                Search
              </Button>
            </div>
          </CardContent>
        ...
        {error ? (<div className="text-center text-sm text-red-500">{error}</div>) : null}

        {searched && (<div className="space-y-4">
            {loadingDoctors ? (<div className="text-center py-12 text-muted-foreground">Loading doctors...</div>) : filteredDoctors.length === 0 ? (<div className="text-center py-12 text-muted-foreground">
                ...
                {filteredDoctors.map((doctor) => {
                    ...
                    return (<Card key={doctor._id} className="border shadow-sm">
                      <CardContent className="p-5 space-y-3">
                        ...
                        <Button className="w-full" onClick={() => navigate(`/user/doctors/${doctor._id}`)}>
