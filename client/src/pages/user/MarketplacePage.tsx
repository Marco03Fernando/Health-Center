  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {categories.map(c => (
          <Button key={c.value} variant={category === c.value ? 'default' : 'outline'} size="sm" onClick={() => setCategory(c.value)} className="shrink-0">
            {c.label}
          </Button>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {filtered.map(p => (
            <ProductCard key={p.id} product={p} onAddToCart={(product) => { addToCart(product); toast.success(`${product.name} added to cart`); }} />
          ))}
        </div>
      )}
    </div>
