...
const STATUS_FILTERS: { label: string; value: LabBookingStatus | "all" }[] = [
  { label: "All", value: "all" },
...
type RawBooking = {
  ...
  diagnosticTest?: { _id?: string; name?: string } | null;
...
function mapBooking(b: RawBooking): UiLabBooking {
  ...
  return {
    ...
    status: (b.appointmentStatus || "CONFIRMED") as LabBookingStatus,
  };
}
...
const LabBookingsPage = () => {
  ...
  const [bookings, setBookings] = useState<UiLabBooking[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState<LabBookingStatus | "all">("all");
  ...
  async function loadBookings() {
    try {
      ...
      const items: RawBooking[] = Array.isArray(res?.data)
        ? res.data
        : Array.isArray(res)
        ? res
        : [];
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="relative">
        ...
        <Input
          placeholder="Search by test name or lab location..."
          className="pl-10"
          value={search}
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {filtered.map((booking) => (
            <Card
              ...
              onClick={() => navigate(`/user/lab-bookings/${booking.id}`)}
            >
