...
import { MapPin, Briefcase, ArrowLeft, CheckCircle2, Loader2, CalendarDays, Ban, CalendarClock, } from "lucide-react";
const DAY_LABELS = {
    mon: "Mon",
    tue: "Tue",
    wed: "Wed",
...
function isDateAllowedBySchedule(date, workingDays, holidayDates) {
    if (!date)
        return false;
...
function getNextAllowedDate(startDate, workingDays, holidayDates, limit = 120) {
...
const DoctorProfilePage = () => {
        ...
        async function loadDoctor() {
            ...
            try {
                ...
                if (!ignore) {
                    ...
                    const nextValidDate = getNextAllowedDate(getTodayDate(), nextDoctor.workingDays || [], nextDoctor.holidayDates || []);
    ...
    const workingDays = useMemo(() => normalizeWorkingDays(doctor?.workingDays), [doctor?.workingDays]);
    const holidayDates = useMemo(() => normalizeHolidayDates(doctor?.holidayDates), [doctor?.holidayDates]);
    ...
    useEffect(() => {
        ...
        async function loadSlots() {
            ...
            try {
                ...
                const available = items.filter((s) => !s.isBooked && s.isActive !== false);
                if (!ignore) {
                    setSlots(available.map((s) => ({
                        _id: s._id || s.id,
                        date: s.date,
    ...
    async function handleConfirm() {
        if (!doctor || !selectedSlotObj)
            return;
        if (!isSelectedDateAllowed) {
            setError(selectedDateReason || "Selected date is not available for this doctor.");
            return;
        }
        const resolvedCenterId = typeof doctor.centerId === "object"
    ...
    const centerName = typeof doctor.centerId === "object"
    ...
    return (<div className="space-y-5 animate-fade-in">
          ...
          <div className="flex gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-muted-foreground">
                ...
                {(doctor.startTime || doctor.endTime) && (<span className="flex items-center gap-1">
                    <Briefcase className="w-3.5 h-3.5"/>
                    {doctor.startTime || "--:--"} - {doctor.endTime || "--:--"}
                  </span>)}

                {doctor.sessionTime ? (<span className="flex items-center gap-1">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                {doctor.isActive === false ? (<Badge variant="destructive" className="rounded-full">
                    Inactive
                  </Badge>) : (<Badge className="rounded-full">Available</Badge>)}
              </div>

              {holidayDates.length > 0 ? (<p className="text-xs text-muted-foreground mt-3">
      ...
      {step === "info" && (<Button className="w-full" onClick={() => setStep("book")} disabled={doctor.isActive === false}>
          Book Appointment
        </Button>)}
      ...
      {step === "book" && (<Card>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <Input type="date" value={date} onChange={(e) => setDate(e.target.value)} min={getTodayDate()}/>
            </div>
            ...
            {!isSelectedDateAllowed ? (<div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-sm text-amber-700 dark:text-amber-400">
                <div className="flex items-start gap-2">
                  {holidayDates.includes(date) ? (<Ban className="w-4 h-4 mt-0.5 shrink-0"/>) : (<CalendarClock className="w-4 h-4 mt-0.5 shrink-0"/>)}
                  <div>{selectedDateReason}</div>
                </div>
              </div>) : null}

            {loadingSlots ? (<div className="text-sm text-muted-foreground">Loading available slots...</div>) : date ? (<div>
                <label className="text-sm font-medium text-foreground mb-2 block">
                  Available Slots
                </label>

                {!isSelectedDateAllowed ? (<div className="text-sm text-muted-foreground">
                    Select a valid working day to see available slots.
                  </div>) : slotLabels.length === 0 ? (<div className="text-sm text-muted-foreground">
                    No available slots for this date.
                  </div>) : (<SlotPicker slots={slotLabels} selected={slot} onSelect={setSlot}/>)}
              </div>) : null}
            ...
            {date && slot && isSelectedDateAllowed && (<Button className="w-full" onClick={() => setStep("confirm")}>
                Continue
              </Button>)}
          </CardContent>
        </Card>)}
      ...
      {step === "confirm" && (<Card>
          ...
          <CardContent className="space-y-3">
            <div className="space-y-2 text-sm">
              ...
              {note ? (<div className="flex justify-between gap-4">
