...
import { Search, Plus, Clock, Phone, DollarSign, Edit, Building2, MapPin, Stethoscope, CalendarDays, UserRound, Loader2, } from "lucide-react";
...
function mapDoctor(item) {
    ...
    return {
        ...
        phone: item.phone || "",
        centerId: typeof item.centerId === "string" ? item.centerId : String(centerObject?._id || ""),
...
export default function DoctorsPage() {
    ...
    const [showSlotsDialog, setShowSlotsDialog] = useState(null);
    const [newDoctor, setNewDoctor] = useState(initialCreateForm);
    const [editDoctor, setEditDoctor] = useState(initialEditForm);
    ...
    const specializations = useMemo(() => [...new Set(doctorsList.map((d) => d.specialization).filter(Boolean))], [doctorsList]);
    ...
    const doctorCenterSuggestions = useMemo(() => {
        ...
        centers.forEach((center) => {
            if (center.name?.trim())
                names.add(center.name.trim());
        });
        doctorsList.forEach((doc) => {
            if (doc.centerName?.trim())
                names.add(doc.centerName.trim());
        });
    ...
    const filteredDoctorCenterSuggestions = useMemo(() => {
        ...
        if (!q)
            return doctorCenterSuggestions;
        return doctorCenterSuggestions.filter((name) => name.toLowerCase().includes(q));
    }, [doctorCenterSuggestions, doctorCenterFilter]);
    const getCenterDisplayName = (doc) => {
        if (doc.centerName && doc.centerName.trim())
            return doc.centerName;
    ...
    const handleCenterInputChange = (value) => {
        setCenterSearch(value);
        const exactMatch = centers.find((center) => center.name.trim().toLowerCase() === value.trim().toLowerCase());
        setNewDoctor((prev) => ({
            ...prev,
    ...
    const toggleActive = async (doctor) => {
        try {
            ...
            setDoctorsList((prev) => prev.map((d) => (d.id === doctor.id ? { ...d, isActive: !d.isActive } : d)));
        }
        catch (err) {
    ...
    return (<div className="space-y-8 p-1 md:p-2">
                ...
                <div className="space-y-6 py-6">
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <Input className="h-11 rounded-xl" value={newDoctor.fullName} onChange={(e) => setNewDoctor({ ...newDoctor, fullName: e.target.value })} placeholder="Enter full name"/>
                    </div>

                    <div className="space-y-2">
                      <Label>Doctor Display Name</Label>
                      <Input className="h-11 rounded-xl" value={newDoctor.name} onChange={(e) => setNewDoctor({ ...newDoctor, name: e.target.value })} placeholder="Enter display name"/>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      <Label>Email</Label>
                      <Input type="email" className="h-11 rounded-xl" value={newDoctor.email} onChange={(e) => setNewDoctor({ ...newDoctor, email: e.target.value })} placeholder="Enter email address"/>
                    </div>

                    <div className="space-y-2">
                      <Label>Password</Label>
                      <Input type="password" className="h-11 rounded-xl" value={newDoctor.password} onChange={(e) => setNewDoctor({ ...newDoctor, password: e.target.value })} placeholder="Enter password"/>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      <Label>Phone</Label>
                      <Input className="h-11 rounded-xl" value={newDoctor.phone} onChange={(e) => setNewDoctor({ ...newDoctor, phone: e.target.value })} placeholder="Enter phone number"/>
                    </div>

                    <div className="space-y-2">
                      <Label>Center</Label>
                      <Input list="center-options" className="h-11 rounded-xl" value={centerSearch} onChange={(e) => handleCenterInputChange(e.target.value)} placeholder={loadingCenters ? "Loading centers..." : "Type center name"} autoComplete="off"/>
                      <datalist id="center-options">
                        {filteredCenters.map((center) => (<option key={center._id} value={center.name}/>))}
                      </datalist>
            ...
            : "Type and select a valid center name"}
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <Input className="h-11 rounded-xl" value={newDoctor.specialization} onChange={(e) => setNewDoctor({ ...newDoctor, specialization: e.target.value })} placeholder="Enter specialization"/>
                    </div>

                    <div className="space-y-2">
                      <Label>Clinic</Label>
                      <Input className="h-11 rounded-xl" value={newDoctor.clinic} onChange={(e) => setNewDoctor({ ...newDoctor, clinic: e.target.value })} placeholder="Enter clinic / department"/>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-4 md:grid-cols-3">
                    <div className="space-y-2">
                      <Label>Fee</Label>
                      <Input type="number" className="h-11 rounded-xl" value={newDoctor.fee} onChange={(e) => setNewDoctor({ ...newDoctor, fee: e.target.value })} placeholder="Consultation fee"/>
                    </div>

                    <div className="space-y-2">
                      <Label>Start Time</Label>
                      <Input type="time" className="h-11 rounded-xl" value={newDoctor.startTime} onChange={(e) => setNewDoctor({ ...newDoctor, startTime: e.target.value })}/>
                    </div>

                    <div className="space-y-2">
                      <Label>End Time</Label>
                      <Input type="time" className="h-11 rounded-xl" value={newDoctor.endTime} onChange={(e) => setNewDoctor({ ...newDoctor, endTime: e.target.value })}/>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      ...
                      <Input type="number" className="h-11 rounded-xl" value={newDoctor.sessionTime} onChange={(e) => setNewDoctor({ ...newDoctor, sessionTime: e.target.value })} placeholder="30"/>
                    </div>

                    <div className="space-y-2">
                      <Label>Slot Date</Label>
                      <Input type="date" className="h-11 rounded-xl" value={newDoctor.date} onChange={(e) => setNewDoctor({ ...newDoctor, date: e.target.value })}/>
                    </div>
                  </div>

                  <div className="flex justify-end pt-2">
                    <Button onClick={handleAddDoctor} disabled={submitting || loadingCenters} className="h-11 rounded-xl px-6">
        ...
        <CardContent className="p-5 md:p-6">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="flex w-full flex-col gap-3 xl:w-auto xl:flex-row">
              <div className="relative w-full xl:min-w-[300px]">
                ...
                <Input placeholder="Search doctor, phone, clinic, center..." className="h-11 rounded-xl pl-10" value={search} onChange={(e) => setSearch(e.target.value)}/>
              </div>

              <div className="relative w-full xl:min-w-[260px]">
                <MapPin className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground"/>
                <Input list="doctor-center-filter-options" placeholder="Filter by center name..." className="h-11 rounded-xl pl-10" value={doctorCenterFilter} onChange={(e) => setDoctorCenterFilter(e.target.value)} autoComplete="off"/>
                <datalist id="doctor-center-filter-options">
                  {filteredDoctorCenterSuggestions.map((name) => (<option key={name} value={name}/>))}
                </datalist>
              </div>
              ...
              <Select value={specFilter} onValueChange={setSpecFilter}>
                ...
                <SelectContent>
                  ...
                  {specializations.map((s) => (<SelectItem key={s} value={s}>
                      {s}
                    </SelectItem>))}
                </SelectContent>
              </Select>
          ...
          <div className="mt-6">
            {loading ? (<div className="flex items-center justify-center rounded-2xl border border-dashed py-16 text-muted-foreground">
                ...
                {filtered.map((doc) => (<Card key={doc.id} className="group h-full rounded-2xl border shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg">
                    <CardHeader className="space-y-4 pb-4">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <Badge variant={doc.isActive ? "default" : "secondary"} className="rounded-full px-3 py-1 text-[11px]">
                    ...
                    <CardContent className="pt-0">
                      <div className="flex items-center justify-between border-t pt-4">
                        <div className="flex items-center gap-3 rounded-xl border bg-background px-3 py-2">
                          <Switch checked={doc.isActive} disabled={togglingId === doc.id} onCheckedChange={() => toggleActive(doc)}/>
                    ...
                    : doc.isActive
                        ...
                        <div className="flex items-center gap-2">
                          <Button variant="outline" size="sm" className="h-9 rounded-xl" onClick={() => setSelectedDoctor(doc)}>
                            <Edit className="mr-1.5 h-3.5 w-3.5"/>
                            Edit
                          </Button>

                          <Button variant="ghost" size="sm" className="h-9 rounded-xl" onClick={() => setShowSlotsDialog(doc)}>
      ...
      <Dialog open={!!selectedDoctor} onOpenChange={() => setSelectedDoctor(null)}>
        <DialogContent className="max-w-3xl rounded-3xl p-0 overflow-hidden">
          ...
          {selectedDoctor && (<div className="max-h-[80vh] overflow-y-auto px-6 pb-6">
              <div className="space-y-6 py-6">
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    <Label>Name</Label>
                    <Input className="h-11 rounded-xl" value={editDoctor.name} onChange={(e) => setEditDoctor({ ...editDoctor, name: e.target.value })}/>
                  </div>

                  <div className="space-y-2">
                    <Label>Center ID</Label>
                    <Input className="h-11 rounded-xl" value={editDoctor.centerId} onChange={(e) => setEditDoctor({ ...editDoctor, centerId: e.target.value })}/>
                  </div>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    <Label>Specialization</Label>
                    <Input className="h-11 rounded-xl" value={editDoctor.specialization} onChange={(e) => setEditDoctor({ ...editDoctor, specialization: e.target.value })}/>
                  </div>

                  <div className="space-y-2">
                    <Label>Clinic</Label>
                    <Input className="h-11 rounded-xl" value={editDoctor.clinic} onChange={(e) => setEditDoctor({ ...editDoctor, clinic: e.target.value })}/>
                  </div>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  <div className="space-y-2">
                    <Label>Fee</Label>
                    <Input type="number" className="h-11 rounded-xl" value={editDoctor.fee} onChange={(e) => setEditDoctor({ ...editDoctor, fee: e.target.value })}/>
                  </div>

                  <div className="space-y-2">
                    <Label>Phone</Label>
                    <Input className="h-11 rounded-xl" value={editDoctor.phone} onChange={(e) => setEditDoctor({ ...editDoctor, phone: e.target.value })}/>
                  </div>

                  <div className="space-y-2">
                    <Label>Session Time</Label>
                    <Input type="number" className="h-11 rounded-xl" value={editDoctor.sessionTime} onChange={(e) => setEditDoctor({ ...editDoctor, sessionTime: e.target.value })}/>
                  </div>
                </div>
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    <Label>Start Time</Label>
                    <Input type="time" className="h-11 rounded-xl" value={editDoctor.startTime} onChange={(e) => setEditDoctor({ ...editDoctor, startTime: e.target.value })}/>
                  </div>

                  <div className="space-y-2">
                    <Label>End Time</Label>
                    <Input type="time" className="h-11 rounded-xl" value={editDoctor.endTime} onChange={(e) => setEditDoctor({ ...editDoctor, endTime: e.target.value })}/>
                  </div>
                </div>

                <div className="flex justify-end pt-2">
                  <Button onClick={handleUpdateDoctor} disabled={submitting} className="h-11 rounded-xl px-6">
      ...
      <Dialog open={!!showSlotsDialog} onOpenChange={() => setShowSlotsDialog(null)}>
        <DialogContent className="max-w-2xl rounded-3xl p-0 overflow-hidden">
          ...
          {showSlotsDialog && (<div className="px-6 pb-6">
              <div className="space-y-5 py-5">
                ...
                <div className="rounded-2xl border p-4">
                  ...
                  {generateSlots(showSlotsDialog).length > 0 ? (<div className="flex flex-wrap gap-2">
                      {generateSlots(showSlotsDialog).map((slot) => (<Badge key={slot} variant="outline" className="rounded-full px-3 py-1.5 text-xs">
                          {slot}
                        </Badge>))}
