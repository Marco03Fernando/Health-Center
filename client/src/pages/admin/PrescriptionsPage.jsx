...
import { Search, Pill, Download, Loader2, FileText, Building2, User, Stethoscope, CalendarDays, } from "lucide-react";
...
export default function PrescriptionsPage() {
    ...
    async function handleDownloadPdf(id, prescriptionNo) {
        try {
            setDownloading(true);
        ...
        <CardContent className="p-5 md:p-6">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="flex w-full flex-col gap-3 xl:w-auto xl:flex-row">
              <div className="relative w-full xl:min-w-[360px]">
                ...
                <Input placeholder="Search by prescription no, doctor, patient, center, or diagnosis..." className="h-11 rounded-xl pl-10" value={search} onChange={(e) => setSearch(e.target.value)}/>
              </div>
          ...
          <div className="mt-6">
            {loading ? (<div className="flex items-center justify-center rounded-2xl border border-dashed py-16 text-muted-foreground">
              ...
              </div>) : error ? (<div className="rounded-2xl border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">
                {error}
                ...
                {filtered.map((rx) => {
                const displayDate = rx.appointmentId?.slotId?.date || rx.createdAt || rx.updatedAt;
                return (<Card key={rx._id} className="cursor-pointer rounded-2xl border shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md" onClick={() => setSelected(rx)}>
                      <CardContent className="p-5">
                        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                          <div className="flex min-w-0 items-start gap-4">
                            ...
                            <div className="min-w-0 flex-1 space-y-3">
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                <Badge variant="secondary" className={`rounded-full border px-3 py-1 text-[11px] ${statusStyles[rx.status] || "bg-muted text-muted-foreground border-transparent"}`}>
                          ...
                          <div className="flex items-center justify-end">
                            <Button type="button" variant="outline" className="h-10 rounded-xl" onClick={(e) => {
                        e.stopPropagation();
                        setSelected(rx);
                    }}>
                              View Details
              ...
              <div className="space-y-6 py-4">
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  ...
                  <div className="rounded-2xl border bg-muted/30 p-4">
                    ...
                    <p className="mt-1 break-words text-sm font-semibold">
                      {formatDate(selected.appointmentId?.slotId?.date ||
                selected.createdAt ||
                selected.updatedAt)}
                    </p>
                  </div>
                ...
                ? `${selected.appointmentId.slotId.startTime}${selected.appointmentId?.slotId?.endTime
                ...
                : "-"}
                  ...
                  <div className="rounded-2xl border bg-muted/30 p-4">
                    ...
                    <div className="mt-2">
                      <Badge variant="secondary" className={`rounded-full border px-3 py-1 text-[11px] ${statusStyles[selected.status] || "bg-muted text-muted-foreground border-transparent"}`}>
                ...
                <div className="rounded-2xl border p-4">
                  ...
                  {(selected.items || []).length > 0 ? (<div className="space-y-3">
                      {selected.items.map((item, i) => (<div key={i} className="flex flex-col gap-3 rounded-xl border bg-muted/20 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    </div>) : (<div className="rounded-xl border border-dashed p-4 text-sm text-muted-foreground">
                      No medicines added.
                    </div>)}
                </div>

                {selected.notes ? (<div className="rounded-2xl border p-4">
                ...
                <div className="flex justify-end pt-2">
                  <Button type="button" onClick={() => handleDownloadPdf(selected._id, selected.prescriptionNo)} disabled={downloading} className="h-11 shrink-0 rounded-xl px-5">
