...
export default function CentersPage() {
  ...
  const getLocationText = (center: Center) => {
    if (center.address && center.district) return `${center.address}, ${center.district}`;
    if (center.address) return center.address;
    if (center.district) return center.district;
    return "Location not available";
  };

  const CenterCard = ({ center }: { center: Center }) => (
    <Card className="group h-full overflow-hidden rounded-2xl border border-border/60 bg-card shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg">
        ...
        <div className="grid grid-cols-2 gap-3 border-t pt-4">
          <div className="flex items-center justify-between rounded-xl border bg-background px-3 py-3">
            ...
            <Switch
              ...
              onCheckedChange={() => toggleActive(center._id)}
            />
          </div>
          ...
          <div className="flex items-center justify-between rounded-xl border bg-background px-3 py-3">
            ...
            <Switch
              checked={center.isFeatured}
              onCheckedChange={() => toggleFeatured(center._id)}
            />
          </div>
        </div>
      </CardContent>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setNewCenter((prev) => ({ ...prev, district: e.target.value }))}
        ...
        <CardContent className="p-5 md:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="relative w-full lg:max-w-md">
              ...
              <Input
                placeholder="Search by center name, district, address, or phone..."
                value={search}
