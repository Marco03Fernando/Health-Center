...
import { Search, Pill, Download, Loader2, FileText, User, Stethoscope, CalendarDays, } from "lucide-react";
const statusStyles = {
...
export default function CenterPrescriptionsPage() {
    ...
    async function fetchPrescriptions() {
        try {
            ...
            const filtered = centerId
                ? all.filter((p) => {
                    const cid = p.centerId && typeof p.centerId === "object" ? p.centerId._id : p.centerId;
                    return cid === centerId;
                })
                : all;
    ...
    return (<div className="space-y-8 p-1 md:p-2">
      <div className="rounded-3xl border bg-gradient-to-br from-background to-muted/30 p-6 shadow-sm md:p-8">
        ...
        ].map(({ label, value }) => (<Card key={label} className="rounded-2xl border shadow-none">
          ...
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between mb-5">
            ...
            <div className="flex w-full flex-col gap-3 xl:w-auto xl:flex-row">
              <div className="relative w-full xl:min-w-[320px]">
                ...
                <Input placeholder="Search by patient, doctor, diagnosis..." className="h-11 rounded-xl pl-10" value={search} onChange={(e) => setSearch(e.target.value)}/>
              </div>
                ...
                return (<Card key={rx._id} className="cursor-pointer rounded-2xl border shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md" onClick={() => setSelected(rx)}>
                    <CardContent className="p-5">
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <Button variant="outline" size="sm" className="h-10 rounded-xl shrink-0" onClick={(e) => { e.stopPropagation(); setSelected(rx); }}>
                          View Details
                        </Button>
                      </div>
                    </CardContent>
      ...
      <Dialog open={!!selected} onOpenChange={() => setSelected(null)}>
        <DialogContent className="max-w-3xl rounded-3xl p-0 overflow-hidden">
          ...
          {selected && (<div className="max-h-[80vh] overflow-y-auto px-6 pb-6">
              <div className="space-y-6 py-4">
                ...
                { label: "Date", value: formatDate(selected.appointmentId?.slotId?.date || selected.createdAt) },
                {
            ...
            ].map(({ label, value }) => (<div key={label} className="rounded-2xl border bg-muted/30 p-4">
                  ...
                  <div className="rounded-2xl border bg-muted/30 p-4">
                    ...
                    <div className="mt-2">
                      <Badge variant="secondary" className={`rounded-full border px-3 py-1 text-[11px] ${statusStyles[selected.status] || ""}`}>
                ...
                <div className="flex justify-end">
                  <Button onClick={() => handleDownloadPdf(selected._id, selected.prescriptionNo)} disabled={downloading} className="rounded-xl gap-2">
