...
type Booking = {
  ...
  healthCenter?: { _id?: string; name?: string; address?: string; phone?: string } | null;
  diagnosticTest?: {
    _id?: string;
    name?: string;
    description?: string;
...
export default function BookingDetailPage() {
  ...
  const slotDisplayStatus = booking?.slot
    ? getSlotDisplayStatus({
        ...
        status: (booking.slot.status as any) || "AVAILABLE",
      })
    : null;
  ...
  return (
    <div className="space-y-8 p-1 md:p-2">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-2">
            ...
            {!loading && booking && (
              <Badge
                variant="secondary"
                className={`rounded-full border px-3 py-1 text-[11px] ${BOOKING_STATUS_STYLES[booking.appointmentStatus]}`}
              >
      ...
      {loading ? (
        <div className="flex items-center justify-center rounded-2xl border border-dashed py-24 text-muted-foreground">
            ...
            <CardContent className="p-6 space-y-5">
              ...
              <div className="space-y-4">
                <DetailRow
                  icon={Calendar}
                  label="Date"
                  value={formatDate(booking.slot?.slotDate || booking.appointmentDate)}
                />
                <DetailRow
                  icon={Clock}
                  label="Time Slot"
                ...
                {slotDisplayStatus && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <Badge
                        variant="secondary"
                        className={`mt-1 rounded-full border px-3 py-0.5 text-[11px] ${SLOT_STATUS_STYLES[slotDisplayStatus]}`}
                      >
          ...
          <Card className="rounded-3xl border shadow-sm lg:col-span-2">
            <CardContent className="p-6 space-y-5">
              ...
              {booking.diagnosticTest?.instructions && (
                <div className="flex gap-3 rounded-xl border border-warning/30 bg-warning/5 p-4 mt-2">
              ...
              {booking.appointmentStatus === "CONFIRMED" && (
                <div className="flex justify-end pt-2">
                  <Button
                    variant="outline"
                    className="rounded-xl text-destructive hover:text-destructive border-destructive/30"
