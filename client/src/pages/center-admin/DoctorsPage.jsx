...
import { Search, Stethoscope, Phone, Clock, DollarSign, Loader2, UserRound, } from "lucide-react";
...
export default function CenterDoctorsPage() {
    ...
    async function fetchDoctors() {
        try {
            ...
            const all = Array.isArray(data?.items) ? data.items.map(mapDoctor) : [];
    ...
    return (<div className="space-y-8 p-1 md:p-2">
      ...
      <Card className="rounded-3xl border shadow-sm">
        <CardContent className="p-5 md:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between mb-5">
            ...
            <div className="flex w-full flex-col gap-3 md:flex-row md:items-center lg:w-auto">
              <div className="relative w-full md:min-w-[260px]">
                ...
                <Input placeholder="Search by name, specialization..." className="h-11 rounded-xl pl-10" value={search} onChange={(e) => setSearch(e.target.value)}/>
              </div>
              <div className="flex gap-2 flex-wrap">
                {["all", ...specializations].map((s) => (<Button key={s} variant={specFilter === s ? "default" : "outline"} size="sm" onClick={() => setSpecFilter(s)} className="rounded-xl capitalize shrink-0">
                    {s === "all" ? "All" : s}
                  </Button>))}
              </div>
            </div>
          ...
          {loading ? (<div className="flex items-center justify-center rounded-2xl border border-dashed py-16 text-muted-foreground">
              ...
              {filtered.map((doc) => (<Card key={doc.id} className="cursor-pointer rounded-2xl border shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md" onClick={() => setSelected(doc)}>
                  <CardContent className="p-5">
                      ...
                      <div className="flex min-w-0 items-start gap-4">
                        ...
                        <div className="min-w-0 flex-1 space-y-2">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <Badge variant="secondary" className={`rounded-full border px-3 py-1 text-[11px] ${doc.isActive
                    ...
                    : "bg-muted text-muted-foreground border-transparent"}`}>
                      ...
                      <div className="flex items-center gap-3">
                        {togglingId === doc.id ? (<Loader2 className="h-4 w-4 animate-spin text-muted-foreground"/>) : (<Switch checked={doc.isActive} onCheckedChange={() => handleToggle(doc)} onClick={(e) => e.stopPropagation()}/>)}
                        <Button variant="outline" size="sm" className="rounded-xl" onClick={(e) => { e.stopPropagation(); setSelected(doc); }}>
                          View
                        </Button>
                      </div>
                    </div>
      ...
      <Dialog open={!!selected} onOpenChange={() => setSelected(null)}>
        <DialogContent className="max-w-lg rounded-3xl p-0 overflow-hidden">
          ...
          {selected && (<div className="grid gap-4 p-6 md:grid-cols-2">
              {[
