...
export default function CenterAdminLoginPage() {
    ...
    if (isAuthenticated)
        return <Navigate to="/center-admin" replace/>;
      ...
      <Card className="w-full max-w-md shadow-lg">
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            {error && (<div className="rounded-xl border border-destructive/30 bg-destructive/5 px-4 py-2 text-sm text-destructive">
                {error}
              </div>)}
            <div className="space-y-2">
              <Label htmlFor="email">Email</Label>
              <Input id="email" type="email" placeholder="admin@center.com" value={email} onChange={(e) => setEmail(e.target.value)} required/>
            </div>
            <div className="space-y-2">
