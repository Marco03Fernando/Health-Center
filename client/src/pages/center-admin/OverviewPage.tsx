...
type Doctor = { _id?: string; isActive?: boolean; centerId?: string | { _id?: string } };
type BookingStatus = "CONFIRMED" | "COMPLETED" | "CANCELLED";
type LabBooking = { _id?: string; appointmentStatus?: BookingStatus; healthCenter?: string | { _id?: string } };
type Prescription = { _id?: string; status?: string; centerId?: string | { _id?: string } };
type Slot = { _id?: string; status?: string; center?: string };

function getCenterId(val: string | { _id?: string } | undefined, fallback: string): boolean {
  if (!val) return false;
...
export default function CenterOverviewPage() {
  ...
  async function loadAll() {
    try {
      ...
      if (doctorRes.status === "fulfilled") {
        const all: Doctor[] = Array.isArray(doctorRes.value?.items) ? doctorRes.value.items : [];
        setDoctors(centerId ? all.filter((d) => getCenterId(d.centerId, centerId)) : all);
      }
      if (prescriptionRes.status === "fulfilled") {
        const all: Prescription[] = Array.isArray(prescriptionRes.value?.data) ? prescriptionRes.value.data : [];
        setDrescriptions(all, centerId);
      }
      if (bookingRes.status === "fulfilled") {
        const raw = bookingRes.value;
        const all: LabBooking[] = raw?.data || (Array.isArray(raw) ? raw : []);
        setLabBookings(all);
      }
  ...
  function setDrescriptions(all: Prescription[], cid: string) {
    if (!cid) { setPrescriptions(all); return; }
    setPrescriptions(all.filter((p) => getCenterId(p.centerId, cid)));
  }

  const activeDoctors = useMemo(() => doctors.filter((d) => (d as any).isActive).length, [doctors]);
              ...
              {labBookings.slice(0, 5).map((b) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Badge
                      variant="secondary"
                      className={`rounded-full border px-3 py-1 text-[11px] shrink-0 ${statusColors[status] || "bg-muted text-muted-foreground border-transparent"}`}
                    >
