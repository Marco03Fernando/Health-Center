...
export default function CenterLabBookingsPage() {
  ...
  async function fetchBookings() {
    ...
    try {
      ...
      const raw: LabBooking[] = res?.data || res?.appointments || (Array.isArray(res) ? res : []);
      setBookings(raw);
    } catch (err: any) {
                      ...
                      <div className="flex min-w-0 items-start gap-4">
                        ...
                        <div className="min-w-0 flex-1 space-y-2">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <Badge
                              variant="secondary"
                              className={`rounded-full border px-3 py-1 text-[11px] ${STATUS_STYLES[b.appointmentStatus] || "bg-muted text-muted-foreground border-transparent"}`}
                            >
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        {b.appointmentStatus === "CONFIRMED" && (
                          <Button
                            ...
                            onClick={(e) => { e.stopPropagation(); setCancelId(b._id); }}
                          >
                            Cancel
                          </Button>
                        )}
                ...
                {selected.appointmentStatus === "CONFIRMED" && (
                  <div className="flex justify-end">
                    <Button
                      variant="outline"
                      className="rounded-xl text-destructive hover:text-destructive border-destructive/30"
                      onClick={() => setCancelId(selected._id)}
                    >
                      Cancel Booking
                    </Button>
                  </div>
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              onClick={() => { if (cancelId) handleCancel(cancelId); }}
              disabled={cancelling}
            >
