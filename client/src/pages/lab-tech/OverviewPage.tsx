...
export default function LabTechOverviewPage() {
  ...
  const todayBookings = useMemo(
    () =>
      bookings.filter(
        (b) => isToday(b.slot?.slotDate) || isToday(b.appointmentDate)
      ).length,
    [bookings]
  );
  const upcomingBookings = useMemo(
    () => bookings.filter((b) => b.appointmentStatus === "CONFIRMED").length,
    [bookings]
  );
  const completedTests = useMemo(
    () => bookings.filter((b) => b.appointmentStatus === "COMPLETED").length,
    [bookings]
  );
                  ...
                  >
                    <div className="min-w-0">
                      <p className="truncate text-sm font-medium">
                        {(b as any).diagnosticTest?.name || "Lab Test"}
                      </p>
