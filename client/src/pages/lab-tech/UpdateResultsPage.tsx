...
export default function UpdateResultsPage() {
  ...
  const [condition, setCondition] = useState<"normal" | "severe" | "unknown">("unknown");
    ...
    try {
      ...
      if (bookingsRes.status === "fulfilled") {
        const confirmed = bookingsRes.value.filter(
          (b) => b.appointmentStatus === "CONFIRMED"
        );
        setBookings(confirmed);
        if (preselectedBookingId) {
          const pre = confirmed.find((b) => b._id === preselectedBookingId);
          if (pre) openResultForm(pre);
        }
      }
      if (typesRes.status === "fulfilled") setTestTypes(typesRes.value);
    } catch (err: any) {
  ...
  function openResultForm(booking: LabBooking) {
  ...
  function handleTestTypeChange(typeId: string) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const incomplete = resultParams.some(
      (p) => !p.name.trim() || p.value === "" || !p.unit.trim()
    );
    if (incomplete) {
      setSubmitError(
        "Please fill in all parameter fields (name, value, unit)."
      );
      return;
    }
    try {
  ...
  const selectedTestType = useMemo(
    () => testTypes.find((t) => t._id === selectedTestTypeId),
  ...
  return (
    <div className="space-y-8 p-1 md:p-2">
          ...
          {loadingData ? (
            ...
            <div className="space-y-3">
              {filteredBookings.map((b) => (
                <Card
                  key={b._id}
                  className="rounded-2xl border shadow-none transition-all hover:shadow-sm"
                >
                  <CardContent className="p-5">
                        ...
                        <div className="min-w-0 space-y-1">
                          ...
                          <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                            ...
                            <span className="flex items-center gap-1">
                              ...
                              {formatDate(
                                b.slot?.slotDate || b.appointmentDate
                              )}
                            </span>
                          </div>
                        </div>
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && closeResultForm()}
      >
              ...
              <div className="space-y-6 py-4">
                            ...
                            <div className="space-y-1.5">
                              ...
                              <div className="flex gap-2">
                                <Input
                                  ...
                                  className="h-9 rounded-xl text-sm"
                                />
                                {!selectedTestType && resultParams.length > 1 && (
                                  <Button
                                    type="button"
                                    variant="ghost"
                                    size="sm"
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={condition}
                    onValueChange={(v) =>
                      setCondition(v as "normal" | "severe" | "unknown")
                    }
                  >
                    <SelectTrigger className="h-11 rounded-xl">
                      <SelectValue />
