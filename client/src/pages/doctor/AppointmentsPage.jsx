...
import { Clock, Calendar as CalIcon, CalendarDays, Loader2, Stethoscope, CheckCircle2, Lock, CircleOff, CalendarClock, Ban, } from "lucide-react";
...
export default function Appointments() {
    ...
    const availableCount = useMemo(() => sortedSlots.filter((slot) => getSlotStatus(slot) === "available").length, [sortedSlots]);
    const bookedCount = useMemo(() => sortedSlots.filter((slot) => getSlotStatus(slot) === "booked").length, [sortedSlots]);
    const inactiveCount = useMemo(() => sortedSlots.filter((slot) => getSlotStatus(slot) === "inactive").length, [sortedSlots]);
    ...
    return (<div className="space-y-8 p-1 md:p-2">
        ...
        <div className="mt-4 grid gap-4 xl:grid-cols-2">
            ...
            <CardContent className="p-5">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {workingDays.length ? (workingDays.map((day) => (<Badge key={day} variant="outline" className="rounded-full px-3 py-1">
                      {DAY_LABELS[day] || day}
                    </Badge>))) : (<span className="text-sm font-medium text-muted-foreground">-</span>)}
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="rounded-2xl border shadow-none">
            <CardContent className="p-5">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {holidayDates.length ? (holidayDates.slice(0, 6).map((date) => (<Badge key={date} variant="outline" className="rounded-full px-3 py-1">
                      {formatDateLabel(date)}
                    </Badge>))) : (<span className="text-sm font-medium text-muted-foreground">No holidays set</span>)}
                {holidayDates.length > 6 ? (<Badge variant="outline" className="rounded-full px-3 py-1">
      ...
      <Card className="rounded-3xl border shadow-sm">
        ...
        <CardContent className="pt-3">
          {sortedSlots.length === 0 ? (<div className="rounded-2xl border border-dashed p-10 text-center">
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                ...
                const statusUI = status === "booked"
                    ? {
                        badge: "Booked",
                        icon: <Lock className="h-4 w-4 text-amber-600 dark:text-amber-400"/>,
                        cardClass: "border-amber-500/30 bg-amber-500/10 hover:bg-amber-500/15",
                ...
                return (<div key={slot._id || slot.id || `${slot.startTime}-${index}`} className={`rounded-2xl border p-4 transition-colors ${statusUI.cardClass}`}>
                      ...
                      <div className="mt-4 flex items-center justify-between gap-3">
                        <Badge variant="outline" className={`rounded-full text-[11px] ${statusUI.badgeClass}`}>
                          {statusUI.badge}
                        </Badge>
