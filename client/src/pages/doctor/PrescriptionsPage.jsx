...
import { Search, ClipboardList, Download, Eye, Loader2, FileText, Pill, UserRound, Mail, CalendarDays, } from "lucide-react";
...
function getApiBaseUrl() {
    const viteUrl = typeof import.meta !== "undefined" &&
        ...
        : "";
    const nextUrl = typeof process !== "undefined" &&
...
export default function Prescriptions() {
    ...
    const filteredPrescriptions = useMemo(() => {
        ...
        return prescriptions.filter((item) => {
            ...
            return (prescriptionNo.includes(q) ||
                diagnosis.includes(q) ||
                patientName.includes(q) ||
                patientEmail.includes(q));
        });
    }, [prescriptions, search]);
    const issuedCount = prescriptions.filter((item) => (item.status || "issued") === "issued").length;
    const dispensedCount = prescriptions.filter((item) => item.status === "dispensed").length;
    const totalMedicines = prescriptions.reduce((sum, item) => sum + (item.items?.length || 0), 0);
    ...
    return (<div className="space-y-8 p-1 md:p-2">
      <div className="rounded-3xl border bg-gradient-to-br from-background to-muted/30 p-6 shadow-sm md:p-8">
        <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div className="relative w-full xl:max-w-sm">
            ...
            <Input placeholder="Search by prescription no, patient, diagnosis..." value={search} onChange={(e) => setSearch(e.target.value)} className="h-11 rounded-xl pl-10"/>
          </div>
        </div>
      ...
      <Card className="rounded-3xl border shadow-sm">
        ...
        <CardContent className="pt-3">
          {loading ? (<div className="flex items-center justify-center rounded-2xl border border-dashed py-16 text-muted-foreground">
            ...
            </div>) : filteredPrescriptions.length === 0 ? (<div className="rounded-2xl border border-dashed p-10 text-center">
              ...
              {filteredPrescriptions.map((item) => (<Card key={item._id} className="rounded-2xl border shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md">
                  <CardContent className="p-5">
                      ...
                      <div className="flex min-w-0 items-start gap-4">
                        ...
                        <div className="min-w-0 flex-1 space-y-3">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <Badge variant="outline" className={`rounded-full text-xs ${getStatusClass(item.status || "issued")}`}>
                              {item.status || "issued"}
                            </Badge>
                          </div>
                    ...
                    : ""}
                      ...
                      <div className="flex shrink-0 flex-wrap gap-2 xl:justify-end">
                        <Button variant="outline" size="sm" onClick={() => handleViewDetails(item._id)} disabled={detailsLoading} className="h-10 rounded-xl px-4">
                          {detailsLoading ? (<Loader2 className="mr-2 h-4 w-4 animate-spin"/>) : (<Eye className="mr-2 h-4 w-4"/>)}
                          View
                        </Button>

                        <Button size="sm" onClick={() => handleDownloadPdf(item)} disabled={downloadingId === item._id} className="h-10 rounded-xl px-4">
                          {downloadingId === item._id ? (<Loader2 className="mr-2 h-4 w-4 animate-spin"/>) : (<Download className="mr-2 h-4 w-4"/>)}
                          {downloadingId === item._id ? "Downloading..." : "Download PDF"}
                        </Button>
                      </div>
                    </div>

                    {item.notes ? (<div className="mt-4 rounded-xl border bg-muted/20 p-3 text-sm text-muted-foreground">
      ...
      <Dialog open={!!selectedPrescription} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-4xl rounded-3xl p-0 overflow-hidden">
          ...
          {selectedPrescription && (<div className="max-h-[80vh] overflow-y-auto px-6 pb-6">
              <div className="space-y-6 py-6">
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  ...
                  <div className="rounded-2xl border bg-muted/30 p-4">
                    ...
                    <div className="mt-2">
                      <Badge variant="outline" className={`rounded-full text-xs ${getStatusClass(selectedPrescription.status || "issued")}`}>
                ...
                {selectedPrescription.notes ? (<div className="rounded-2xl border p-4">
                ...
                <div className="rounded-2xl border p-4">
                  ...
                  {!selectedPrescription.items || selectedPrescription.items.length === 0 ? (<div className="rounded-xl border border-dashed p-4 text-sm text-muted-foreground">
                      No medicines added.
                    </div>) : (<div className="space-y-3">
                      {selectedPrescription.items.map((medicine, index) => (<div key={`${medicine.medicineName || "medicine"}-${index}`} className="rounded-xl border bg-muted/20 p-4">
                        ...
                        medicine.duration ||
                        medicine.quantity ||
                        medicine.instructions) && (<div className="mt-3 grid gap-2 text-sm text-muted-foreground md:grid-cols-2">
                              {medicine.dosage ? (<div>
                                  <span className="font-medium text-foreground">Dosage:</span>{" "}
                                  {medicine.dosage}
                                </div>) : null}

                              {medicine.frequency ? (<div>
                                  <span className="font-medium text-foreground">Frequency:</span>{" "}
                                  {medicine.frequency}
                                </div>) : null}

                              {medicine.duration ? (<div>
                                  <span className="font-medium text-foreground">Duration:</span>{" "}
                                  {medicine.duration}
                                </div>) : null}

                              {medicine.quantity ? (<div>
                                  <span className="font-medium text-foreground">Quantity:</span>{" "}
                                  {medicine.quantity}
                                </div>) : null}

                              {medicine.instructions ? (<div className="md:col-span-2">
                ...
                <div className="flex justify-end pt-2">
                  <Button onClick={() => handleDownloadPdf(selectedPrescription)} disabled={downloadingId === selectedPrescription._id} className="h-11 rounded-xl px-5">
                    {downloadingId === selectedPrescription._id ? (<Loader2 className="mr-2 h-4 w-4 animate-spin"/>) : (<Download className="mr-2 h-4 w-4"/>)}
