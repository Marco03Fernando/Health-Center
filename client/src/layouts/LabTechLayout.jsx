      ...
      <SidebarContent>
        <SidebarGroup>
          <SidebarGroupLabel className="px-0 py-0">
            <Link
              to="/"
              className="text-sidebar-primary flex items-center gap-2 px-3 py-4 text-base font-bold w-full"
            >
...
export default function LabTechLayout() {
  const { isAuthenticated, isLoading, user, refreshAuth } = useLabTech();
  ...
  if (!isAuthenticated) return <Navigate to="/lab-tech/login" replace />;
