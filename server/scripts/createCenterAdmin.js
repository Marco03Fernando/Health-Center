...
async function run() {
  ...
  if (!centerId) {
    console.error(
      "Usage: node createCenterAdmin.js <centerId> [email] [password] [name]"
    );
    process.exit(1);
  }
  ...
  if (existing) {
    ...
    existing.role = "center-admin";
    existing.centerId = new mongoose.Types.ObjectId(centerId);
    existing.isActive = true;

    await existing.save();

    console.log("Updated existing center admin:");
    console.log({
      id: existing._id.toString(),
      email: existing.email,
      role: existing.role,
      centerId: existing.centerId.toString(),
    });
  } else {
